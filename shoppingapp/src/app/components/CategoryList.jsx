...
const CategoryList = () => {
  ...
  if (error) return <p className="error-text">Error fetching categories: {error.message}</p>;
          ...
          {categories.map((category) => {
            ...
            return (
              <div key={category.id} className="category-card">
                <Image
                  src={randomProduct ? randomProduct.imageUrl : "/placeholder.jpg"} // Random product image or fallback
                  alt={category.name}
                  width={270}
                  height={325}
